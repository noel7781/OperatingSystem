...
scheduler(void)
{
  ...
  for(;;){
	...
	for(p = (p==&ptable.proc[NPROC]) ? ptable.proc : p; p < &ptable.proc[NPROC]; ++p){
		...
		if(p->priority != pri || p->state != RUNNABLE) {
		  continue;
		}
...
yield(void)
{
  ...
  g_process_queue.Data[0].move += g_process_queue.Data[0].stride * myproc()->qtime;
  myproc()->qtime = 0;
  MakeHeap(g_process_queue.size - 1);
...
int set_cpu_share(int percent) {
  myproc()->tickets = percent;
  priority_cnt[myproc()->priority]--;
  myproc()->priority = 3;
  priority_cnt[myproc()->priority]++;
  PushQueue(myproc());
  return percent;
...
void inc_total_time(struct proc* p) {
	p->ttime++;
	if((p->priority == 0 && p->ttime >= 5) || (p->priority == 1 && p->ttime >= 15)){
		priority_cnt[p->priority]--;
		p->priority++;
		priority_cnt[p->priority]++;
		p->qtime = 0;
